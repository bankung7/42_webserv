...
| | 100 : Continue
Only a part of the request has been received by the server, but as long as it has not been rejected, the client should continue with request. |
| 2XX | Success: it means the action was successfully received, understood, and accepted. |
| | 201 : Created
The request is completed, and a new resource is create. |
| | 202 : Accepted
The request is accepted for processing, but the processing is not completed. |
The server is returning partial data of the size requested. The server must specify the range included in the response with the Content-Range header. |
